    ...
    try:
        ...
        if not df_trend.empty and not df_ibge.empty:
            ...
            if len(common_idx) > 6:
                ...
                line_micro = base.mark_line(color='
                    y=alt.Y('Novas Empresas', axis=alt.Axis(title='Novas Empresas', titleColor='
                )
                line_macro = base.mark_line(color='
                    y=alt.Y('Indústria (IBGE)', axis=alt.Axis(title='Benchmark Nacional (IBGE)', titleColor='
                )
        ...
        if not has_correlation:
            if not df_trend.empty:
                ...
                if 'companies' in df_fb.columns:
                    ...
                    df_fb['Empresas'] = "-"
                chart_fb = alt.Chart(df_fb).mark_line(point=True, color='
        ...
        if not df_ibge.empty:
            ...
            with c_pulse:
                ...
                bar_pulse = alt.Chart(df_pulse).mark_bar().encode(
                    ...
                    color=alt.condition(alt.datum.value > 0, alt.value('
                    tooltip=[
                        alt.Tooltip('date:T', title='Data', format='%b/%Y'),
            ...
            with c_trend:
                ...
                area_trend = alt.Chart(df_trend).mark_area(line={'color':'
                    gradient='linear',
                    stops=[alt.GradientStop(color='white', offset=0), alt.GradientStop(color='
                    x1=1, x2=1, y1=1, y2=0
                ), opacity=0.5).encode(
                    x=alt.X('date:T', axis=alt.Axis(format='%Y'), title=None),
                    y=alt.Y('value:Q', title='%'),
